...
int isAsciiDigit(int x) {
    ...
    // return (((~x + 1) + 0x2f) >> 31 & 1)  ^ (((~x + 1) + 0x3a) >> 31 & 1);
}
/* 
...
int isLessOrEqual(int x, int y) {
    ...
    // 符号位相同情况的判断
    int sign_same = !(sign_x ^ sign_y) & ((x + ~y) >> 31);
    // 符号位情况不同的判断
...
int howManyBits(int x) {
    ...
    tmp = tmp >> bit1;
    bit0 = (!!tmp);
    tmp = bit16 + bit8 + bit4 + bit2 + bit1 + bit0 + 1;
    return isZero | tmp;
}
//float
/* 
